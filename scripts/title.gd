...
func _ready() -> void:
    ...
    for session in sessions:
        ...
        restore_control.remove_session.connect(_on_remove_session_button_pressed.bind(session.timestamp))
...
func _on_remove_session_button_pressed(timestamp: int) -> void:
    for child in $VBoxContainer.get_children():
        if timestamp == child.get_node("HBoxContainer/PileCard").pile.timestamp:
...
func _remove_confirmed(child: Node) -> void:
    ...
    File_IO.delete_session(child.get_node("HBoxContainer/PileCard").pile.timestamp)
